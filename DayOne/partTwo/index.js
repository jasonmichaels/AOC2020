...
const findTarget = (data) => {
   ...
   */
  const numbers = data.flatMap(i => data.flatMap(j => data.map(k => [i, j, k]))).filter(([x, y, z]) => x + y + z === 2020);
...
console.log(findTarget(testData)); // 241861950
console.log(findTarget(allData)); // 145245270